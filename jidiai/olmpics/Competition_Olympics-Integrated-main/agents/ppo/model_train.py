...
from agents.ppo.distributions_train import Bernoulli, Categorical, DiagGaussian, MultiDiagGaussian
from agents.ppo.ppo_utils import init
...
class Policy(nn.Module):
    ...
    def act(self, inputs, rnn_hxs, masks, deterministic=False):
        ...
        if deterministic:
            action1 = np.clip(dist[0].mode().cpu(), self.action_space[0].low, self.action_space[0].high).to(torch.device('cuda'))
            action2 = np.clip(dist[1].mode().cpu(), self.action_space[1].low, self.action_space[1].high).to(torch.device('cuda'))
        else:
            action1 = np.clip(dist[0].sample().cpu(), self.action_space[0].low, self.action_space[0].high).to(torch.device('cuda'))
            action2 = np.clip(dist[1].sample().cpu(), self.action_space[1].low, self.action_space[1].high).to(torch.device('cuda'))
