    ...
    def stepPhysics(self, actions_list, step = None):
        ...
        while True:
            ...
            if collision_wall_target is not None and collision_circle_target is None:
                if self.print_log:
                    print('HIT THE WALL!')
                if current_agent_idx in [0, 1]:
                    reward[current_agent_idx] -= 5
                ...
                if self.print_log:
                    print('HIT THE BALL!')
                if current_circle_idx in [0, 1]:
                    if target_circle_idx == 2:
                        reward[current_circle_idx] += 20
                    else:
                        reward[current_circle_idx] += 5
