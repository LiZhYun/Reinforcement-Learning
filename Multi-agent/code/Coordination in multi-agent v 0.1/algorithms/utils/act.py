...
class ACTLayer(nn.Module):
    ...
    def kl_divergence(self, mu1, mu2, std1, std2, probs1, probs2):
        if mu1.grad_fn == None:
            probs2 = probs2.detach()
            kl = self.kl_approx(probs2, probs1)
        else:
            logstd1 = torch.log(std1)
            mu2 = mu2.detach()
            std2 = std2.detach()
            logstd2 = torch.log(std2)
            # kl divergence between old policy and new policy : D( pi_old || pi_new )
            # pi_old -> mu0, logstd0, std0 / pi_new -> mu, logstd, std
            # be careful of calculating KL-divergence. It is not symmetric metric
            kl = logstd1 - logstd2 + \
                (std2.pow(2) + (mu2 - mu1).pow(2)) / \
                (2.0 * std1.pow(2)) - 0.5  # 高斯分布的KL散度
