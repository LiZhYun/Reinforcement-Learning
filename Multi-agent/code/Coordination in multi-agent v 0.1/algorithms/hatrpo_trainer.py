...
class HATRPO():
    ...
    def kl_divergence(self, obs, rnn_states, action, masks, available_actions, active_masks, new_actor, old_actor):
        ...
        if mu.grad_fn==None:
            ...
            kl =  logstd - logstd_old  + (std_old.pow(2) + (mu_old - mu).pow(2)) / (2.0 * std.pow(2)) - 0.5  # 高斯分布的KL散度
    ...
    def train(self, buffer, update_actor=True):
        ...
        if self._use_popart:
            ...
            advantages = buffer.returns[:-1] - buffer.value_preds[:-1]  # Generalized advantage estimation
    ...
    def prep_training(self): # 设置为训练模式
        self.policy.actor.train()
        self.policy.critic.train()

    def prep_rollout(self):  # 设置为评估模式
