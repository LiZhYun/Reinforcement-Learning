...
class Runner(object):
    def __init__(self, config):
        ...
        self.use_centralized_V = self.all_args.use_centralized_V # false
        self.use_obs_instead_of_state = self.all_args.use_obs_instead_of_state # true
        ...
        self.trainer = []
        self.buffer = []
        self.intention_buffer = []  # 可以先进行pre-training
        self.intention_buffer_eval = []
        for agent_id in range(self.num_agents):
            # algorithm
            tr = TrainAlgo(self.all_args, self.policy[agent_id], self.num_agents, agent_id, device=self.device)
            # buffer
            share_observation_space = self.envs.share_observation_space[agent_id] if self.use_centralized_V else self.envs.observation_space[agent_id]
            bu = SeparatedReplayBuffer(self.all_args,
                                       self.envs.observation_space[agent_id],
                                       share_observation_space,
                                       self.envs.action_space[agent_id], self.envs.n_agents)
            self.buffer.append(bu)
            self.trainer.append(tr)
            self.intention_buffer.append(Normal(torch.zeros((self.all_args.n_rollout_threads * self.envs.n_agents, self.all_args.intention_size)), 
            torch.ones((self.all_args.n_rollout_threads * self.envs.n_agents, self.all_args.intention_size))))
            
            self.intention_buffer_eval.append(Normal(torch.zeros((self.all_args.n_eval_rollout_threads * self.envs.n_agents, self.all_args.intention_size)), 
            torch.ones((self.all_args.n_eval_rollout_threads * self.envs.n_agents, self.all_args.intention_size))))
        ...
        for agent_id in range(self.num_agents):
            ...
            next_value = self.trainer[agent_id].policy.get_values(self.buffer[agent_id].share_obs[-1], 
                                                                self.buffer[agent_id].rnn_states_critic[-1],
                                                                  self.buffer[agent_id].masks[-1], self.intention_buffer[agent_id], use_intention=True)
    ...
    def train(self):
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            self.buffer[agent_id].update_factor(factor)  # 存储已更新的重要性比值
            ...
            if self.all_args.algorithm_name == "hatrpo":
                old_actions_logprob, _, _, _, _ =self.trainer[agent_id].policy.actor.evaluate_actions(self.buffer[agent_id].obs[:-1].reshape(-1, *self.buffer[agent_id].obs.shape[2:]),
                                                            ...
                                                            available_actions,
                                                            self.buffer[agent_id].active_masks[:-1].reshape(-1, *self.buffer[agent_id].active_masks.shape[2:]), intention=self.intention_buffer[agent_id], use_intention=True)
            else:
                old_actions_logprob, _, _, _, _ = self.trainer[agent_id].policy.actor.evaluate_actions(self.buffer[agent_id].obs[:-1].reshape(-1, *self.buffer[agent_id].obs.shape[2:]),
                                                            self.buffer[agent_id].rnn_states[0:1].reshape(-1, *self.buffer[agent_id].rnn_states.shape[2:]),
                                                            self.buffer[agent_id].actions.reshape(-1, *self.buffer[agent_id].actions.shape[2:]),
                                                            ...
                                                            available_actions,
                                                            self.buffer[agent_id].active_masks[:-1].reshape(-1, *self.buffer[agent_id].active_masks.shape[2:]), intention=self.intention_buffer[agent_id], use_intention=True)
            train_info = self.trainer[agent_id].train(self.buffer[agent_id], self.intention_buffer[agent_id], intention_clip=True, long_short_clip=False)
