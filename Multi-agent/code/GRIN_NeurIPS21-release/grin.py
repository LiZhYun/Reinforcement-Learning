...
class GRIN(nn.Module):
    ...
    def build_graph(self, past, future=None):
        ...
        loc_past, vel_past = past # [bz, timesteps, num_var, feat]
        if future:
        ...
        data_feat = data_feat.transpose(2, 1) # [bz, num_var, timesteps, feat]
    ...
    def forward(self, graph):
        ...
        zG_rv, zA_rv = self.encoder(graph, feats) # (batchsize * num_atoms, z_dim)
        zG = zG_rv.rsample([self.num_sample]).transpose(1, 0)
        zA = zA_rv.rsample([self.num_sample]).transpose(1, 0)  # (batchsize * num_atoms, 1, z_dim)
        graph.ndata['zG'] = zG
        graph.ndata['zA'] = zA
        loc_pred = self.decoder.forward(  # [batch_size * num_var, timesteps, input_size]
