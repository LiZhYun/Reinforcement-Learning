...
class NodeEncoder(nn.Module):
    ...
    def forward(self, graph, inputs):
        ...
        _, x = self.rnn(x)  # Encoder Eq. 1  # (num_layers * num_directions, batch, hidden_size) (1, batchsize * num_atoms, hidden_size)
        x = x.squeeze(0)   # (batchsize * num_atoms, hidden_size)
        x = self.gcn(graph, x)

        zG_mu = self.zG_mu_enc(x)  # Encoder Eq. 5 # (batchsize * num_atoms, z_dim)
...
class NodeDecoder(nn.Module):
    ...
    def forward(self, graph, inputs, pred_steps, forecast=False):
        ...
        inputs = inputs.transpose(0, 1).contiguous()  # [timesteps, batch_size * num_var, 1, input_size]
